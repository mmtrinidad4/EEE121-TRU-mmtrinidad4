...
while(true){
    ...
    if(syntax[0] == "PARK" && syntax[1] != "_" && syntax[2] != "_" && syntax[3] != "_" && syntax[4] != "_" && syntax[5] == "_"){
        if(curSize <= 500){
            vector<string> car; car.resize(10);        
        ...
        if(logSize <= 1000){
            ...
            for(int x = 0; x < curSize; x+=1){
                if(cars_parked[x][0] == syntax[1]){
                    ...
                    cars_parked[x][3] = to_string(hours); cars_parked[x][4] = "P" + to_string(cost);
        ...
        break;}
            ...
            cout << endl;
            bool swapped;
            for (int i = 0; i < cars_parked.size() - 1; i++) {
                swapped = false;
                for (int j = 0; j < cars_parked.size() - i - 1; j++) {
                    if (cars_parked[j][9] > cars_parked[j + 1][9]) {  
                        swap(cars_parked[j], cars_parked[j + 1]);
                        swapped = true;
                    }
                }
